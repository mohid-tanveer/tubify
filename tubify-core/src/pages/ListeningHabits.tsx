...
function SafeBarChart({ data, options }: BarChartProps) {
  return (
    <div className="chart-container w-full h-full">
      <Bar data={data} options={options} key={`bar-chart-${Date.now()}`} />
    </div>
  )
}
...
function SafeLineChart({ data, options }: LineChartProps) {
  return (
    <div className="chart-container w-full h-full">
      <Line data={data} options={options} key={`line-chart-${Date.now()}`} />
    </div>
  )
}

// create a cache key from parameters
const createCacheKey = (type: string, timeRange: string) =>
  `${type}_${timeRange}`

// cache storage key
const STORAGE_CACHE_KEY = "tubify_listening_habits_client_cache"
const STORAGE_CACHE_TIMESTAMP_KEY =
  "tubify_listening_habits_client_cache_timestamp"
...
export default function ListeningHabits() {
  ...
  const [cachedData, setCachedData] = useState<
    Record<string, ListeningHabitsData>
  >(() => {
    try {
      ...
      if (
        cacheTimestamp &&
        Date.now() - parseInt(cacheTimestamp, 10) < CLIENT_CACHE_TTL
      ) {
  ...
  const cacheKey = useMemo(
    () =>
      createCacheKey(`${artistsTimeRange}_${genresTimeRange}`, trendsTimeRange),
    [artistsTimeRange, genresTimeRange, trendsTimeRange],
  )

  // function to fetch data and update cache
  const fetchDataForParams = useCallback(
    async (
      params: {
